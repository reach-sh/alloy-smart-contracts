...
export const machine = Reach.App(() => {
  ...
  const [R, toksTkn, rows, loadedRows, tokensLoaded] = parallelReduce([
    digest(0),
    0,
    rPicker,
    0,
    ...
    .define(() => {
      ...
      const updtRow = (user, r, ctc) => {
        const newArr = r.nftCtcs.set(r.loadedIndex, Maybe(Contract).Some(ctc));
        check(
          r.loadedIndex + 1 <= newArr.length,
          'make sure row does not exceed bounds'
        );
        rowMap[user] = {
          nftCtcs: newArr,
          loadedIndex: r.loadedIndex + 1,
          rowToksTkn: 0,
        };
        return () => r.loadedIndex + 1;
      };
      const chkLoad = user => {
      ...
      const chkValidRow = rNum => {
        ...
        const rowIndex = rN % loadedRows;
        const maxIndex = loadedRows - 1;
        check(rowIndex <= maxIndex, 'row array bounds check');
        const [row, updatedRows] = getIfrmArr(
          rows,
          rowIndex,
          maxIndex,
          Address
        ...
        switch (row) {
          ...
          case Some: {
            ...
            const areAllTaken = pR.rowToksTkn + 1 == pR.nftCtcs.length
            const nRows = areAllTaken ? updatedRows : rows
            return [d, row, rN, nRows];
          }
        }
      };
        ...
        return () => {
          switch (uR) {
            case Some: {
              ...
              check(loadedIndex == nftCtcs.length, 'loaded nfts are in bounds');
              check(rowToksTkn + 1 < nftCtcs.length, 'nfts taken are in bounds');
              const nonTakenLength = loadedIndex - rowToksTkn;
              const nftCtcIndex = rN % nonTakenLength;
              ...
              return [user, sSlot, newArr, uR, rowForUsr];
            }
            case None: {
              assert(true);
    ...
    .while(toksTkn == 0 ? true : toksTkn < tokensLoaded)
    .paySpec([payToken])
    .api(
      api.loadRow,
      (_, _) => {
      ...
      (contract, rNum, notify) => {
        ...
        notify([loadedRows, r.loadedIndex + 1]);
        return [nR, toksTkn, rows, loadedRows, tokensLoaded + nToksLoaded];
      }
    )
    .api(
      api.checkIfLoaded,
      ...
      notify => {
        ...
        notify(isRfull);
        return [R, toksTkn, nRows, nLoaded, tokensLoaded];
      }
    )
    .api(
      api.insertToken,
      ...
      (rNum, notify) => {
        const [rD, row, rN, nRows] = chkValidRow(rNum);
        uMap[this] = rD;
        notify(row);
        return [rN, toksTkn + 1, nRows, loadedRows, tokensLoaded];
      }
    )
    .api(
      api.turnCrank,
      ...
      (rNum, notify) => {
        const rN = getRNum(rNum);
        const [user, slot, newArr, uR, rowForUsr] = chckValidCtc(this, rNum)();
        check(slot !== thisContract, 'slot is not valid contract');
        uMap[this] = {
          row: user.row,
        ...
        notify(slot);
        return [rN, toksTkn, rows, loadedRows, tokensLoaded];
      }
    )
    .api(
      api.finishTurnCrank,
